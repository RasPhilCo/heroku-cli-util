...
function apiMiddleware (response, cb) {
  ...
  let handleFailureOrig = this.handleFailure
  this.handleFailure = function (res, buffer) {
    let body
    try {
      body = this.parseBody(buffer)
    } catch (e) {
      handleFailureOrig.call(this, res, buffer)
    }

    if (res.statusCode === 403 && body.id === 'two_factor') {
      // TODO: do the prompt for real just wanting feedback earlier
      this.options.headers = Object.assign({}, this.options.headers, {'Heroku-Two-Factor-Code': '2fa'})
      this.request()
    } else {
      handleFailureOrig.call(this, res, buffer)
    }
